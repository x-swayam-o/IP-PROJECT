...
const prices = ["12,599", "20,999", "14,699"];
...
const descriptions = [
    [
        "Being a piece of history to the city's urban streets as you walk into Nike Jordan Max Aura 3 men's sneakers. Inspired by the rich Jordanian heritage, this model has the energy of basketball shoes and a look that changes the perception of the classic style.",
    ],
    [
        "Nike Air Max 95 men's sneakers move you with the strength and fluidity inspired by the anatomy of the human body. Thwe central sole forms the basis of these sneakers, while the structured side panels give a solid and stable construction. Flexible incisions in the sole allow your feet to move naturally.",
    ],
    [
...
//Functions
//Retriving image from the folder path
...
//Reset Active States to buttons
...
for ( let i = 0; i < thumb.length; i++) {
    thumb[i].addEventListener("click", (e) => {
        ...
        //Adding the fade in animation in the shoe
        animate(imgContainer, 550, "fade 500ms ease-in-out");
    });
}

for(let i = 0; i < colorBtn.length; i++) {
    // setting up colors to the color button
    assignColors(i, shoe);
...
function slider(shoe) {
    ...
    price.innerText = "Rs." + prices[shoe - 1];
    resetStars(shoe - 1);
...
prev.addEventListener("click", () => {
    //Derement img id
    shoe--;
    //Check if slide goes below the first, and reset it to the last slide
    if (shoe < 1) {
        shoe = pag.length;
    }
...
next.addEventListener("click", () => {
    //Increment img id
    shoe++;
    //Check if slider goes above the slides length, and reset it to the first 
    if(shoe > pag.length) {
        shoe = 1;
    }
    //Run the slider function
